...
export async function createTravelImage(visited) {
  ...
  const scale = Math.min(600 / bounds.width, 490 / bounds.height);
  ctx.save();
  ctx.translate(540 + (600 - bounds.width * scale) / 2, 60 + (490 - bounds.height * scale) / 2);
...
function ImagePreview({ image, onClose, onTextShare }) {
  ...
  async function share() {
    const file = new File([image.blob], "tabinuri-travel.png", { type: "image/png" });
  ...
  return <dialog ref={dialog} className="share-image-dialog" aria-labelledby="share-image-title" onCancel={e => { e.preventDefault(); onClose(); }}>
    ...
    <img src={image.url} alt={`タビヌリ：47都道府県中${image.count}県訪問の日本地図`} width="1200" height="630" />
    <div className="share-actions">
      <a href={image.url} download="tabinuri-travel.png">画像を保存</a>
...
export default function ShareImage({ visited, onTextShare }) {
  ...
  async function generate() {
    ...
    try {
      ...
      setImage({ blob, url: URL.createObjectURL(blob), count: snapshot.length });
