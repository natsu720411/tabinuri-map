...
export function validateRequest(value) {
  ...
  if (value.revisionRequest != null && typeof value.revisionRequest !== "string") throw new Error("旅程の修正希望を確認してください。");
  const revisionRequest = typeof value.revisionRequest === "string" ? value.revisionRequest.trim() : "";
  if (revisionRequest.length > 1000) throw new Error("旅程の修正希望は1000文字以内にしてください。");
  ...
  let lockedItems = [];
  if (value.lockedItems != null) {
    if (!Array.isArray(value.lockedItems) || value.lockedItems.length > 40) throw new Error("固定する予定を確認してください。");
    lockedItems = value.lockedItems.map(item => {
      if (!item || !Number.isInteger(item.day) || item.day < 1 || item.day > dates.length || typeof item.time !== "string" || !/^([01]\d|2[0-3]):[0-5]\d$/.test(item.time) || typeof item.title !== "string" || !item.title.trim() || item.title.length > 180 || typeof item.memo !== "string" || item.memo.length > 1400) throw new Error("固定する予定を確認してください。");
      return { day: item.day, time: item.time, title: item.title.trim(), memo: item.memo.trim() };
    });
  }
  let existingItinerary = null;
  if (revisionRequest || value.existingItinerary != null) {
    if (!revisionRequest || !value.existingItinerary) throw new Error("修正する旅程と修正希望を確認してください。");
    existingItinerary = validateItinerary(value.existingItinerary, dates.length);
  }
  return { plan, mood: value.mood, pace: value.pace, transportStyle: value.transportStyle, requestNote, revisionRequest, existingItinerary, lockedItems, dates };
}
